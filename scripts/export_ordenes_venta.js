...
const EFFI_URL   = 'https://effi.com.co/app/orden_v';
...
(async () => {
  ...
  try {
    ...
    const fileEncabezados = path.join(EXPORT_DIR, `ordenes_venta_encabezados_${fecha}.xlsx`);
    ...
    const fileDetalle = path.join(EXPORT_DIR, `ordenes_venta_detalle_${fecha}.xlsx`);
    ...
    await page.screenshot({ path: `/exports/error_ordenes_venta_${Date.now()}.png` });
    process.exit(1);
  } finally {
    await browser.close();
  }
