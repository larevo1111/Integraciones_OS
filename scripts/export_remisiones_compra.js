...
const EXPORT_DIR = '/exports/remisiones_compra';
const EFFI_URL   = 'https://effi.com.co/app/remision_c?vigente=1';
...
(async () => {
  ...
  try {
    console.log('🔄 Navegando a Remisiones de compra...');
    ...
    const fileEncabezados = path.join(EXPORT_DIR, `remisiones_compra_encabezados_${fecha}.xlsx`);
    ...
    const fileDetalle = path.join(EXPORT_DIR, `remisiones_compra_detalle_${fecha}.xlsx`);
    ...
    await page.screenshot({ path: `/exports/error_remisiones_compra_${Date.now()}.png` });
    process.exit(1);
  } finally {
    await browser.close();
  }
