...
const EXPORT_DIR = '/exports/comprobantes_ingreso';
const EFFI_URL   = 'https://effi.com.co/app/ingreso?vigente=1';
...
(async () => {
  ...
  try {
    console.log('🔄 Navegando a Comprobantes de ingreso...');
    ...
    const fileEncabezados = path.join(EXPORT_DIR, `comprobantes_ingreso_encabezados_${fecha}.xlsx`);
    ...
    const fileDetalle = path.join(EXPORT_DIR, `comprobantes_ingreso_detalle_${fecha}.xlsx`);
    ...
    await page.screenshot({ path: `/exports/error_comprobantes_ingreso_${Date.now()}.png` });
    process.exit(1);
  } finally {
    await browser.close();
  }
