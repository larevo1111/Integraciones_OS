  ...
  <div class="op-selector" ref="wrapRef">
    ...
    <div class="op-input-wrap" :class="{ abierto: abierto, 'tiene-valor': !!modelValue }">
      ...
      <div v-if="modelValue && !abierto" class="op-tag">
        <span class="op-tag-num">Rem. {{ modelValue }}</span>
        <span class="op-tag-desc">{{ clienteSeleccionado }}</span>
        <!-- Acciones hover -->
        <div class="op-tag-acciones">
          <a
            class="op-tag-accion"
            :href="`https://effi.com.co/app/remision_v?id=${modelValue}`"
            target="_blank"
            rel="noopener"
            title="Ver en Effi"
            @click.stop
    ...
    <Transition name="opdrop">
      <div v-if="abierto" class="op-dropdown" :style="dropdownStyle">
        ...
        <div v-else-if="!resultados.length" class="op-estado">
          ...
          {{ query ? 'Sin resultados para "' + query + '"' : 'No hay remisiones' }}
        </div>

        <template v-else>
          <div class="op-hint">{{ resultados.length }} remisión{{ resultados.length !== 1 ? 'es' : '' }}</div>
          <div
            v-for="(r, i) in resultados"
            :key="r.id_remision"
            class="op-item"
            :class="{ focused: focoIdx === i, selected: modelValue === r.id_remision }"
            @mouseenter="focoIdx = i"
            @click="seleccionar(r)"
          >
            <div class="op-item-left">
              <span class="op-num">Rem. {{ r.id_remision }}</span>
              <span class="op-articulos">{{ truncar(r.cliente || '—', 55) }}</span>
            </div>
          </div>
        </template>
      </div>
...
async function cargar(q) {
  ...
  try {
    ...
    const data = await api(`/api/gestion/remisiones${params}`)
    resultados.value = data.remisiones || []
  } catch { resultados.value = [] } finally { cargando.value = false }
}

function seleccionar(r) {
  if (!r) return
  emit('update:modelValue', r.id_remision)
  clienteSeleccionado.value = truncar(r.cliente || '', 40)
  query.value = ''
  abierto.value = false
}
...
async function verPdf() {
  ...
  try {
    ...
    const resp  = await fetch(`/api/gestion/remision/${encodeURIComponent(props.modelValue)}/pdf`, {
...
watch(() => props.modelValue, async (val) => {
  if (val && !clienteSeleccionado.value) {
    try {
      const data = await api(`/api/gestion/remision/${encodeURIComponent(val)}`)
      clienteSeleccionado.value = truncar(data.remision?.cliente || '', 40)
    } catch {}
  }
}, { immediate: true })
</script>
