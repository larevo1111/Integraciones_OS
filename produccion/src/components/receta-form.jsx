...
/**
 * Formulario reutilizable de edición de receta.
 * Usado en la página /recetas/:cod y en el modal abierto desde el preview de OP.
 * Maneja: carga, edición de materiales/productos/costos/puntos críticos/observaciones/notas, guardado.
 *
 * Props:
 *  - cod: string (cod_articulo del producto principal de la receta)
 *  - onSaved: (data) => void (opcional — se llama después de guardar exitoso)
 *  - hideNotas: bool (oculta secciones notas + razonamiento; usado en modal compacto)
 *  - onLoaded: (receta) => void (opcional — recibe el objeto receta cargado)
 */
export function RecetaForm({ cod, onSaved, hideNotas = false, onLoaded }) {
  ...
  return (
    <div className="space-y-5">
      {/* Acciones de dirty (Descartar / Guardar) */}
      {dirty && (
        <div className="flex items-center justify-end gap-2 sticky top-0 z-10 bg-background/95 backdrop-blur py-2 border-b border-border -mx-1 px-1">
      ...
      {/* Notas / razonamiento (oculto en el modal compacto) */}
      {!hideNotas && (
