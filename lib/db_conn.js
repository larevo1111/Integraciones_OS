...
// ─── Cargar .env central — FALLBACK si Infisical falla ──────────────
// Desde 2026-05-11, los secrets viven en Infisical. El .env queda como
// fallback durante la transición. Cuando todo esté validado, se elimina.
...
// Carga al startup los secrets de /shared y los expone como process.env.
// Si Infisical falla, las apps siguen funcionando con valores del .env.
let _infisicalLoaded = false
let _infisicalSecrets = null  // cache para SSH keys
async function _bootstrapInfisical() {
  try {
    const secrets = require('./infisical')
    const shared = await secrets.getMany('/shared')
    // NO sobrescribir si ya está en process.env (cargado del .env). Coherente con
    // cargarEnv() que usa `if (!(k in process.env))`. Esto evita desincronizaciones
    // catastróficas si Infisical guarda un password viejo que rompe la conexión:
    // el `.env` (controlado por nosotros y validado contra MariaDB) gana.
    // Doc del incidente: .agent/docs/INCIDENT_2026-05-14_INFISICAL_DB_PASS_VIEJO.md
    for (const [k, v] of Object.entries(shared)) {
      if (!(k in process.env)) process.env[k] = v
    }
    _infisicalSecrets = secrets
    _infisicalLoaded = true
  } catch (e) {
    console.warn(`[db_conn] WARN: Infisical bootstrap falló, usando .env: ${e.message}`)
  }
}
// Promise que resuelve cuando bootstrap termina. Las funciones async esperan.
const _bootstrapPromise = _bootstrapInfisical()
...
const _remotas = {
  ...
  MASTER:      { sshClient: null, tcpServer: null, ready: false, pool: null, reconectando: false },
...
async function _sshKeyFor(prefijo, fallbackPath) {
  // Espera el bootstrap (idempotente — si ya terminó, resuelve inmediato)
  await _bootstrapPromise
  if (_infisicalLoaded && _infisicalSecrets) {
    try {
      const which = prefijo === 'COMUNIDAD' ? 'HOSTINGER' : 'VPS'
      const keyStr = await _infisicalSecrets.getSSHKey(which)
      return Buffer.from(keyStr)
    } catch (e) {
      console.warn(`[db_conn:${prefijo}] SSH key Infisical falló, fallback a path: ${e.message}`)
    }
  }
...
function _conectarSSH(prefijo, cfg) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Promise.resolve wrapper para mantener interfaz sincrónica del callback
    _sshKeyFor(prefijo, cfg.sshKey).then(key => {
      _conectarSSHConKey(prefijo, cfg, key, resolve, reject, st)
    }).catch(reject)
  })
}

function _conectarSSHConKey(prefijo, cfg, key, resolve, reject, st) {
...
// El master se migró de MariaDB a Postgres el 9-may-2026 (otro agente movió
// la BD; el código del Sistema Gestión seguía apuntando a MariaDB).
// Adapter mysql2-compat: las queries siguen usando `?` y `const [rows] = ...`
// El adapter convierte `?` → `$N` y devuelve `[rows]` como mysql2.
// Patrón extensible: si mañana migramos otra BD a Postgres, agregar otra
// entrada en _remotasPg con su prefijo y exponer en API pública.

let _pgLib = null
function _loadPg() {
  if (!_pgLib) _pgLib = require('pg')
  return _pgLib
}
...
function _adapterPg(pool) {
  return {
    async query(sql, params = []) {
      // Convertir ? → $1, $2, $3... (en orden de aparición)
      let i = 0
      const pgSql = sql.replace(/\?/g, () => `$${++i}`)
      const r = await pool.query(pgSql, params)
      // Emular [rows, fields] de mysql2 — el destructuring [rows] funciona igual
...
module.exports = {
  ...
  // Async (mysql2): abren tunnel+pool si aún no listo. Usar en `conectar()` del bootstrap.
  ...
  // Async (postgres): master vive en Postgres desde 2026-05-09
  master()      { return _conectarPg('MASTER') },
  // Sync getters al pool/adapter actual (pueden ser null si aún no conectado)
  ...
  get poolMaster()      { return _remotasPg.MASTER.adapter },
}
